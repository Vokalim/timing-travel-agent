...
function bindDiscovery(data,expanded){
 ...
 output.querySelectorAll('.choose-destination').forEach(button=>button.addEventListener('click',()=>{form.elements.namedItem('destination').value=button.dataset.city;run();}));
}
function formTrip(){const t=Object.fromEntries(new FormData(form));for(const key of ['nights','flightBudget','hotelBudget','rating','totalTripBudgetCny'])t[key]=t[key]===''?null:Number(t[key]);t.destination=t.destination?.trim()||null;t.start=t.start||null;t.end=t.end||null;return t;}
...
async function run(){
 ...
 try{const t=formTrip();if(!t.origin?.trim())throw new Error(tr('请填写出发地，再继续规划。','Add an origin to continue planning.'));if(!t.destination){button.disabled=false;agentStatus.hidden=true;await runDiscovery({fields:{travelIntents:t.travelIntents?.split(',').filter(Boolean)||[]},interpretation:{...t,destinationState:'discovery_required',departureWindowText:t.departureWindowText}},{skip:true});return;}const next=await planTrip(t,mode,{language});if(id!==requestId)return;discoveryResult=undefined;result=next;selected=result.best?.date;render();resultsState.hidden=false;indicator.textContent=sourceLabel(mode);document.querySelector('#result-meta').textContent=mode==='live'?'LIVE · DUFFEL':'DEMO · SEARCH COMPLETE';document.querySelector('#stale').hidden=version===editVersion;resultsState.scrollIntoView({behavior:'smooth',block:'start'});}catch(error){if(id!==requestId)return;if(mode==='live'&&form.elements.namedItem('origin').value.trim())showLiveUnavailable(error.message);else{structured.open=true;document.querySelector('#error').textContent=error.message;}}finally{if(id===requestId){button.disabled=false;agentStatus.hidden=true;button.querySelector('span').textContent=copy[language].searchDates;}}
}

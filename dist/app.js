...
const copy={
 zh:{kicker:'你的旅行，从一个念头开始',heroTitle:'找个时机，出发吧。',heroSubtitle:'从一个想法，到一趟完整旅行。告诉途米你想去哪，或想怎么玩。',askLabel:'这次想怎么走？',aiNote:'目的地未定也没关系',explore:'开始探索',inspireLabel:'这个时节适合',refreshInspiration:'换一换',chipChristmas:'过圣诞',chipBeach:'去看海',chipHiking:'去爬山',chipFood:'吃点好的',chipRelax:'放空几天',chipWeekend:'周末短途',workingTitle:'途米正在为你找……',working1:'理解旅行偏好',working2:'整理时间与预算',working3:'寻找适合的旅行方案',working4:'比较交通与价格',editConditions:'查看 / 修改旅行条件',tripDetails:'旅行条件',confirmHint:'没想好也没关系，途米可以先帮你比较',from:'出发地',to:'目的地',earliest:'最早出发',latest:'最晚出发',duration:'旅行时长（晚）',rating:'酒店最低评分',flightBudget:'往返机票预算',hotelBudget:'每晚酒店预算',preferences:'其他偏好（可选）',searchDates:'继续规划',demoMode:'演示',liveMode:'实时',editRequest:'修改需求',stale:'旅行条件已修改，请重新比较。',howDecision:'途米如何做判断',methodPreferences:'<strong>先理解偏好。</strong> AI 只负责整理可复核的旅行条件；不可用时会明确标记本地解析。',methodMath:'<strong>计算保持确定。</strong> 价格、预算、评分和预订时机建议都由固定规则计算。',methodSource:'<strong>来源保持透明。</strong> 演示模式使用模拟数据；实时模式使用 Duffel 航班，酒店仍为演示数据。',footer:'在出发之前，先找到对的时机。'},
...
function showLiveUnavailable(message,trip){
 ...
 notice.innerHTML=`<strong>${tr('实时航班数据不可用','Live flight data unavailable')}</strong><p>${escape(message)} ${tr('不会用模拟航班替代；住宿区域与行程仍可继续规划。','No mock flight is substituted; stay-area and itinerary planning can continue.')}</p><button type="button" id="use-demo">${tr('切换到演示模式','Use Demo Mode')}</button>`;

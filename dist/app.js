...
function renderDiscovery(data,advance=false){
 ...
 const images=[...visual.heroImages,...visual.scenicImages].filter((item,index,items)=>item&&items.findIndex(other=>other.src===item.src)===index).slice(0,2);
...
function changeMode(){++requestId;result=undefined;independentTrip=undefined;discoveryResult=undefined;discoverySnapshot=null;clarificationDraft=undefined;output.innerHTML='';resultsState.hidden=true;notice.hidden=true;indicator.textContent=sourceLabel();}

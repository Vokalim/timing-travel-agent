...
export function createTemporalContext(preferences,{now=new Date(),language='en'}={}) {
  ...
  if(month){const year=nextMonthYear(month,now).year;return {kind,datePrecision:kind==='relative_period'?'relative':holiday?'holiday':'broad_month',dateDescription:text,basis:'user_requested_period',periodText:text,month,year,season:season(month),holiday,inferredTravelIntents:inferredFor(month,holiday),language};}

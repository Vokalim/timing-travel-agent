...
export function createTemporalContext(preferences,{now=new Date(),language='en'}={}) {
  ...
  if(earliest&&latest) {const month=Number(earliest.slice(5,7));return {kind:'exact_window',basis:'user_provided_dates',earliestDeparture:earliest,latestDeparture:latest,month,year:Number(earliest.slice(0,4)),season:season(month),holiday:null,inferredTravelIntents:inferredFor(month),language};}
  let month=monthFromText(text),holiday=null,kind='broad_period';
  if(/圣诞|聖誕|christmas/i.test(text)){month=12;holiday='christmas';}
  else if(/春节|春節|lunar new year|spring festival/i.test(text)){holiday='spring_festival';if(!month)return {kind:'holiday_soft_context',basis:'user_requested_holiday',periodText:text,month:null,year:null,season:'winter',holiday,inferredTravelIntents:inferredFor(null,holiday),language};}
  ...
  else if(/这个周末|這個週末|this weekend/i.test(text)){const day=now.getUTCDay(),offset=(6-day+7)%7;const start=addDays(new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate())),offset);return {kind:'relative_weekend',basis:'user_relative_period',month:start.getUTCMonth()+1,year:start.getUTCFullYear(),season:season(start.getUTCMonth()+1),holiday:null,inferredTravelIntents:inferredFor(start.getUTCMonth()+1),generatedAnchor:iso(start),language};}
  if(month){const year=nextMonthYear(month,now).year;return {kind,basis:'user_requested_period',periodText:text,month,year,season:season(month),holiday,inferredTravelIntents:inferredFor(month,holiday),language};}
  const currentMonth=now.getUTCMonth()+1;
  return {kind:'current_soft_context',basis:'current_date_soft_context',month:currentMonth,year:now.getUTCFullYear(),season:season(currentMonth),holiday:null,inferredTravelIntents:inferredFor(currentMonth),language};
}
...
export function planRepresentativeDateWindows(context,durationDays,{limit=3}={}) {
  ...
  if(context.kind==='relative_weekend') {const departure=context.generatedAnchor;return [{departure,returnDate:iso(addDays(new Date(departure+'T00:00:00Z'),durationDays)),source:'system_generated',userProvided:false,basis:'relative_period'}];}
  if(context.kind==='exact_window') {
    ...
    return [...new Set(offsets)].slice(0,limit).map(offset=>{const departure=iso(addDays(start,offset));return {departure,returnDate:iso(addDays(new Date(departure+'T00:00:00Z'),durationDays)),source:'system_generated_within_user_window',userProvided:false,basis:'user_provided_dates'};});
  }
  ...
  return [...new Set(days)].slice(0,limit).map(day=>{const departure=`${context.year}-${pad(context.month)}-${pad(day)}`;return {departure,returnDate:iso(addDays(new Date(departure+'T00:00:00Z'),durationDays)),source:'system_generated',userProvided:false,basis:'user_requested_period'};});
}

...
export function planTransportOptions({origin,destination,countryOrRegion,durationDays=null,constraints={},flightVerification={status:'not_checked'}}){
 ...
 if(strong.trainPreferred)scores.train+=30;
 if(strong.selfDrivePreferred)scores.self_drive+=30;
 if(strong.flightPreferred)scores.flight+=25;
 ...
 const options=TRANSPORT_MODES.map(mode=>{
  ...
  const verification=mode==='flight'?{status:flightVerification.status,source:flightVerification.source||null}: {status:applicable?'not_yet_live':'not_applicable',source:null};

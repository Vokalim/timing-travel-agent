...
const normalizeCandidate=value=>{
  ...
  const city=clean(value.city,80),entity=getDestination(city),code=clean(value.iataOrMetroCode,3)?.toUpperCase()||null;
  if(!entity||code&&(code!==entity.directAirportCode||!/^[A-Z]{3}$/.test(code))) return null;
  const strings=key=>Array.isArray(value[key])?value[key].map(item=>clean(item)).filter(Boolean).slice(0,4):[];
  return {city:entity.canonicalName,countryOrRegion:entity.countryNames.en,iataOrMetroCode:entity.directAirportCode,
...
export class LLMDestinationDiscoveryService {
  ...
  async discover(preferences,context={}) {
    ...
    const instructions=`Propose 10 to 16 diverse real destinations for travel discovery. Use only canonical names from this curated destination universe: ${DESTINATION_UNIVERSE.map(entity=>entity.canonicalName).join(', ')}. Consider towns, islands, scenic and historic destinations alongside cities; no airport is required. Exclude the origin. Respect geographyPreference: domestic means mainland China, international means outside mainland China, all should include both where sensible. Return qualitative reasons only. Set iataOrMetroCode to null if unknown; do not guess codes. Never provide flight or hotel prices, availability, popularity rankings, or numeric recommendation scores. Explicit travelIntents outrank inferredTemporalIntents. Never claim current popularity or trending. Keep geography and trip duration practical. Write reasons in the requested language.`;
    const input=JSON.stringify({origin:preferences.origin||null,durationDays:preferences.durationDays||null,totalTripBudgetCny:preferences.totalTripBudgetCny||null,flightBudgetCny:preferences.flightBudgetCny||null,geographyPreference:preferences.geographyPreference||(preferences.domesticAllowed===false?'international':preferences.internationalAllowed===false?'domestic':'all'),avoidOvernightFlights:preferences.avoidOvernightFlights??null,explicitTravelIntents:preferences.travelIntents||[],temporalContext:context});
    try {
      const response=await this.fetchImpl('https://api.openai.com/v1/responses',{method:'POST',signal:controller.signal,headers:{Authorization:`Bearer ${this.apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:this.model,instructions,input,store:false,max_output_tokens:4500,text:{format:{type:'json_schema',name:DESTINATION_DISCOVERY_SCHEMA_NAME,strict:true,schema:DESTINATION_DISCOVERY_SCHEMA}}})});
      ...
      return {candidates:candidates.slice(0,20),source:'openai',parserStatus:'ai',model:this.model};
    } catch(error) {
